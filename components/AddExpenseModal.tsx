...
import { parseReceiptImage } from '../geminiService';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (!parsedData || typeof parsedData.originalAmount !== 'number') {
        ...
        setScanError(
          "We couldn't read the receipt clearly. Please ensure the image is well-lit, not blurry, and shows the total amount and currency clearly."
        );
      } finally {
        setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 max-h-[85vh] overflow-y-auto">
          {/* Scan Section */}
          <div className="mb-8">
            <label className="block text-sm font-medium text-slate-700 mb-2">Smart Scan (THB/NZD)</label>
            ...
            <button 
              onClick={() => fileInputRef.current?.click()}
              disabled={loading}
              className={`w-full py-4 px-6 border-2 border-dashed rounded-2xl transition-all flex flex-col items-center gap-2 group ${
            ...
            >
              {loading ? (
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-indigo-600 font-medium">Gemini is converting...</span>
                </div>
              ) : (
                <>
                  ...
                  <span className="text-slate-600 font-medium">Scan Receipt or Screenshot</span>
                  <span className="text-slate-400 text-xs text-center px-4">Detects THB or NZD and converts automatically</span>
                </>
              )}
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {formData.amount && (
              <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 flex justify-between items-center text-sm animate-in fade-in duration-300">
                <span className="text-emerald-700 font-medium">Converted Value:</span>
                <span className="text-emerald-800 font-bold">${calculateConverted().toFixed(2)} NZD</span>
              </div>
            )}
